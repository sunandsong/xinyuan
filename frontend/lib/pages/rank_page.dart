...
/// 每项还带一句排名依据说明 + 数量单位——之前光秃秃一个字看不出排的是啥。
const _boards = [
  ('wish', '心愿', Icons.star_rounded, '谁点亮的心愿最多', '个心愿'),
  ('task', '任务', Icons.check_circle_rounded, '谁完成的任务最多', '个任务'),
  ('achv', '奖杯', Icons.emoji_events_rounded, '谁解锁的奖杯最多', '枚奖杯'),
  ('place', '足迹', Icons.public_rounded, '谁点亮的地图足迹最多', '处足迹'),
];
...
class _RankPageState extends State<RankPage> {
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 12.5,
                          fontWeight: i == _tab ? FontWeight.w800 : FontWeight.w500,
                          color: i == _tab ? Colors.white : _muted,
                        ),
                      ),
                    ],
  ...
  Widget _body() {
    ...
    if (_error != null) {
      return Center(
        child: Text(_error!, style: const TextStyle(color: _muted, fontSize: 13)),
      );
    }
    ...
    return ListView(
      ...
      children: [for (final u in top) _row(u)],
    );
  }
  ...
  Widget _row(Map<String, dynamic> u) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isMe ? T.accent.withValues(alpha: .18) : Colors.white.withValues(alpha: .05),
        borderRadius: BorderRadius.circular(14),
        border: isMe ? Border.all(color: T.accent.withValues(alpha: .5)) : null,
      ),
      child: Row(
        children: [
          SizedBox(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            Text(
              rank == null ? '还没上榜，加把劲' : '第 $rank 名 · $count${_boards[_tab].$5}',
              style: const TextStyle(
                fontSize: 14.5,
                fontWeight: FontWeight.w800,
                color: Colors.white,
