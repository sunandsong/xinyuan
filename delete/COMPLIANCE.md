...
| "Offer to delete the entire account record, along with associated personal data. You may include additional options, but only offering to temporarily deactivate or disable an account is insufficient." | ✓ 走 `softDeleteUser`：账号 + 心愿 + 任务 + 时光胶囊都删；保留项见下 |
...
| "Keep users informed. If the deletion request will take additional time to complete, let them know. If your app supports in-app purchases, help people understand how billing and cancellations will be handled." | ✓ 立即生效，页面写明；本 App 没有内购 |
| "Apps not operating in highly regulated industries should not require people to make a phone call, send an email, or go through other support flows." | ✓ 填账号 + 密码即可，无人工环节 |
...
| "When you delete an app account based on a user's request, you must also delete the user data associated with that app account." | ✓ 密码校验通过后当场执行，不排队不人工 |
...
`softDeleteUser`（[backend/src/db.ts](../backend/src/db.ts)）给 `users` / `wishes` / `tasks` / `letters`
