...
2. **注销是软删，不是真删**：`softDeleteUser` 只给 users/wishes/tasks/letters 打
   `deleted: true`，logins/events/crashes/feedback 四张表**连标记都没打**，
   注销用户的 IP、设备、行为记录会永久留着。
   - 审核查不出来（黑盒行为跟真删一致：登不上、账号名释放、新账号干净）
   - 真正的风险在监管抽查、Google Play 数据安全表单的书面声明、以及万一数据泄露
   - 注意这条和「注销申请人工执行」是两回事：管理端点了「执行注销」走的还是
     `softDeleteUser`，点完也只是打标记。隐私政策第七节现在写的是「登录日志/崩溃/
     反馈/行为事件会继续保留一段时间，之后随例行清理一并删除」——**那个"例行清理"
     目前并不存在**，是个待兑现的承诺
   - 建议做法：延迟真删（注销时标记+失效，到期后定时任务物理清除全部关联数据），
     时限跟对外承诺的 30 天对齐。约一小时工作量，用户没拍板，先记着
