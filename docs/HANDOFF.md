...
> ✅ **发版流水线已止血（2026-08-17）**：`release.yml` 锁定 `flutter-version: 3.44.8`，
> build-22 构建成功、下载页已更新。b10~b21 积压的功能（本地通知/平板修复/腾讯地图/
> 崩溃监控/图片远程化）都已经发出去了。正经修法（升 Gradle/AGP/Kotlin）还没做，
> 见下面「其他挂起事项」。
...
- **发版流水线的正经修法还没排**（止血已做，见上面恢复方法里的说明）：升
  Gradle→8.14+、AGP→8.11.1+、Kotlin→2.2.20+ 才能解锁到 Flutter 新版；升级
  流程容易连带崩别的，升完要真机验证，另排时间做，不着急。
