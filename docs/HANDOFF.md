...
> ⚠️ **但明天第一件事应该是修发版流水线**（见下面「其他挂起事项」第一条）——
> 功能做了一堆，但 b10 之后的包一直没发出去，合伙人手上还是 8-12 的版本。
...
- 🔴 **最高优先级：APK 发版流水线已断，b10 之后没再发过版。**
  下载页（`/api/releases`）最新还是 **b10（2026-08-12）**，这之后的本地通知、平板修复、
  腾讯地图、崩溃监控、图片远程化**全都没进安装包**（后端云函数和管理端静态站是最新的，
  只有 App 包卡住）。
  - **根因**：`.github/workflows/release.yml` 里写的是 `channel: stable`，**不锁版本**。
    Flutter **3.47.0 正好在 2026-08-12 发布**，之后 10 次构建无差别全挂，跟改了什么无关。
    3.47 有「内置 Kotlin」破坏性变更（AGP 9+ 移除了对 Kotlin Gradle Plugin 的支持），
    本项目的 Gradle 8.12 / AGP 8.9.1 / Kotlin 2.1.0 构建时就一直在警告「即将放弃支持」。
  - **已排除代码问题**：本地（Flutter 3.44.8）构建成功，**干净克隆重建也成功**（42.3MB）。
    纯粹是 CI 跟着上游升级导致的。日志下不下来（需要仓库 admin 权限），所以具体报错行未实锤，
    但时间线和现象高度吻合。
  - **止血方案（建议先做）**：workflow 里加 `flutter-version: 3.44.8` 锁住版本，
    立刻恢复发版，把这两天的功能发给合伙人试。
  - **正经修法（另排时间）**：升 Gradle→8.14+、AGP→8.11.1+、Kotlin→2.2.20+，
    升完要真机验证，这类升级容易连带崩别的。
  - **不管走哪条，`channel: stable` 不锁版本这件事本身就该改**——上游一发新版流水线就可能
    无声挂掉，而发版是最不该静默失败的环节（这次就是静默挂了 10 次没人发现）。
